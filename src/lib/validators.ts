...
export const newJobFormSchema = z
  ...
  .refine(
    (data) => {
      // If the job type is "Custom Service", selectedTasks must not be empty.
      if (data.jobName === "Custom Service") {
        return data.selectedTasks && data.selectedTasks.length > 0;
      }
      return true;
    },
    {
      message: "Please select at least one task for a custom job.",
      path: ["selectedTasks"],
    },
  );
